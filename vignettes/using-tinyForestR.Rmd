...
tf_df <- read_csv("tf_data.csv")
                    
tf_df |>
  head() 
```

As of `r today()` there are `r nrow(tf_df)` planted TFs.
...
nbn_data <- safe_buff(-1.469139, 51.777889, n = 10000)
...
metrics <- calc_bd_metrics(df = nbn_data$result, class = "Insecta")
metrics
